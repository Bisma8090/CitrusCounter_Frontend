...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch('http://192.168.37.218:5000/auth/login', {
        method: 'POST',
        headers: {
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 38, fontWeight: 'bold', color: '#fff',
    ...
    textShadowRadius: 5 },
  subtitle: { fontSize: 18, color: '#fff', marginBottom: 50, textShadowColor: 'rgba(11, 11, 11, 0.3)', textShadowOffset: { width: 2, height: 2 }, textShadowRadius: 5 },
